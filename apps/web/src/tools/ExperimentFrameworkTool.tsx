...
export default function ExperimentFrameworkTool() {
  ...
  const task = useAsyncTask<ExperimentFrameworkResponse>("expFrameworkTaskId", api.experimentFrameworkTask);

  const run = async () => {
    if (!topic.trim()) { setLocalErr("请输入投资主题（如：光通信产业链）"); return; }
    setLocalErr(null);
    try {
      const t = await api.experimentFramework(topic.trim());
      ...
      <Card><CardContent>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <Input
            ...
            className="h-9"
          />
          <Button onClick={() => void run()} disabled={task.running || !topic.trim()} className="h-9">
            {task.running ? "分析中…" : "🚀 开始分析"}
          </Button>
        </div>
        {localErr && <div style={{ color: "#dc2626", fontSize: "0.82rem", marginTop: 6 }}>{localErr}</div>}
        {task.running && <div style={{ color: C.sub, fontSize: "0.8rem", marginTop: 6 }}>⏳ 正在联网搜索并执行 4 层框架分析（约 1-3 分钟，可离开页面）…</div>}
      </CardContent></Card>

      {task.result && (
        <Card><CardContent>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, fontSize: "0.78rem", color: C.muted }}>
            <span>📅 分析日期：{task.result.asOf}</span>
            {task.result.model && <span>· 模型：{task.result.model}</span>}
          </div>
          <MarkdownView>{task.result.report}</MarkdownView>
        </CardContent></Card>
      )}
    </div>
  );
