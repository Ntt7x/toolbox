...
const EXPERIMENT_EC_PROMPT = "你是「欧元/日元全球套利拥挤度预警框架（ec）」的每日研判引擎。用户会提供当日监测数据 JSON，请严格按以下方法论执行。\n\n## 框架核心（原理）\n- B(t)：套利超额收益指标（加速度计）= 已实现汇率变动 − 利差。正数 → 资金涌入；负数 → 资金撤离。\n- Ω：综合泡沫指数（高度计）≈ 五维度合成（汇率极端程度 / 空头拥挤度 / 利差 / 反身性累积 CVAS / 美股估值）。\n- CVAS：累积波动率自强化指数（长期低波环境下的反身性风险累积）。\n- CCV：条件性危机波动率指数（短期波动率极端异动）。\n- 铁律：B 连续两周为负是最高级别资金转向信号（历史 3/3 击中）；Ω>1.6 且至少两个个体维度极端且 B 连续两周为负 → 全面避险。\n\n## 当日监测数据\n以下是服务端采集/计算的当日数据 JSON（已尽力对齐最新；缺失为 null）：\n{data}\n\n## 周度数据\n以下是服务端采集/计算的数据 JSON（已尽力对齐最新；缺失为 null）：\n{data}\n\n## 输出要求\n只输出 JSON（不要任何其他文字）：\n{\n  \"asOf\": \"数据日期\",\n  \"indicators\": { \"b\": 数值或null, \"bTrend\": \"up|down|flat|null\", \"omega\": 数值或null, \"cvas\": 数值或null, \"ccv\": 数值或null, \"signals\": [\"触发信号1\", ...] },\n  \"status\": \"🔴高危|🟡关注|🟢正常\",\n  \"summary\": \"核心判断（3-5 句，含数据依据）\",\n  \"anchors\": [{\"condition\": \"触发条件\", \"status\": \"已触发|未触发|逼近\", \"action\": \"行动建议\"}],\n  \"watchDates\": [{\"date\": \"日期\", \"event\": \"事件\", \"focus\": \"关注点\"}],\n  \"caveats\": [\"数据局限/待核实项\", ...]\n}\n\n数据缺失的指标填 null 并如实说明，绝不臆造。";

// 实验·BMPI 化债牛市（页面2；占位符 {date} {data}）
const EXPERIMENT_BMPI_PROMPT = "你是「化债牛市进度指数（BMPI v4.0）」的周度研判引擎。用户会提供数据 JSON，请按框架计算并输出研判。\n\n## 框架核心\n- R：利率环境（前置约束）——利率是否支持化债（1Y/10Y 国债收益率）。\n- S_L（流动性）：央行是助力还是阻力（逆回购/MLF 净投放方向与规模）。\n- S1：信用修复进度（地方政府拿到钱了吗——城投/置换债进度、股价信号）。\n- S2：信用边际进程（信用从点扩散到链——中游/资本开支相关股价）。\n- S3：信用扩张程度（反向指标——核心资产重估到哪了——PB 水平）。\n- BMPI 合成 = w1*S1 + w2*S2 + w3*S3 + 0.15*R + 0.15*S_L（w1+w2+w3=0.70，按三段阈值动态分配）。\n- 读数分档：<40 🟢正常（牛市早期）｜40-60 🟡关注（中期）｜>60 🔴高危（末期，剩余空间小）。\n\n## 输出要求\n只输出 JSON（不要任何其他文字）：\n{\n  \"asOf\": \"数据日期\",\n  \"indices\": { \"R\": 0-10, \"SL\": 0-10, \"S1\": 0-10, \"S2\": 0-10, \"S3\": 0-10, \"weights\": {\"w1\":0,\"w2\":0,\"w3\":0} },\n  \"bmpi\": 0-100,\n  \"status\": \"🟢正常|🟡关注|🔴高危\",\n  \"summary\": \"核心判断（3-5 句）\",\n  \"details\": [{\"index\": \"S1\", \"score\": 0, \"evidence\": \"依据\", \"confidence\": \"高|中|低\"}],\n  \"watchDates\": [{\"date\": \"日期\", \"event\": \"事件\", \"focus\": \"关注点\"}],\n  \"caveats\": [\"数据局限/待核实项\", ...]\n}\n\n数据缺失如实说明（标 null 或低置信），绝不臆造。";
