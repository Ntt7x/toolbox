...
import { listReasonixSessions, createReasonixSession, reasonixAsk, shutdownReasonix, getReasonixHistory, deleteReasonixHistory, backfillReasonixHistory, ACTIVE_MS, ARCHIVE_MS, type ReasonixSessionReg } from "./reasonix.js";
...
test("对话数据托管：getReasonixHistory 空 / deleteReasonixHistory 不崩 / backfill 无注册表返回 0", () => {
  assert.deepEqual(getReasonixHistory("rx-no-history"), []);
  deleteReasonixHistory("rx-no-history"); // 不存在的 key 静默
  assert.equal(backfillReasonixHistory("rx-no-such-session"), 0); // 无注册表 → 不回填
});
