...
| # | 教训 | 一句话 | 详情 |
|---|---|---|---|
| 1 | **🈲 分号禁则（Windows 最高频错误）** | **bash 工具是 cmd：命令里写 `;` 会被当参数**（`git status;` → unknown switch、`test cache;` → 找不到模块）——**任何命令不带 `;` 结尾/分隔**；toolbox 入口已自动剥离，**裸 git/curl/node 更要避免**；要串命令就拆多次工具调用 | §3 |
| 2 | **手写交互组件优先成熟库** | 滚动/弹层/菜单/折叠一律用 shadcn（ScrollArea/DropdownMenu/Collapsible），别手写轮子——手写反复出 bug（文本重叠/定位漂移/Escape 失效），被用户批"一堆 bug" | §5.4 |
| 3 | **数据安全铁律** | 单测/脚本**绝不允许清空生产 KV**（曾把 todo 20 条清成 0）——涉及写库一律备份/恢复模式 | §6.5 |
| 4 | **🈲 生产 DB 禁止第二进程直连写** | `.file/toolbox.db` 只允许 server 进程写；排查走 `toolbox api` 或**只读打开**；修复先停服 → 备份 → 改 → 校验（低级 agent 直连并发写 → 137 条仓位交易丢失，见 `plans/sqlite-safety.md` + `history/2026-08-31-02.md`） | §6.5 |
| 5 | **数据丢失先冻结现场** | 发现数据丢失第一时间 `dev stop` 复制 db+wal+shm 冻结，再分析（WAL 未 checkpoint 时含已删帧，`toolbox dbdiag wal` 可提取；服务重启/checkpoint 会覆盖） | `plans/sqlite-safety.md` |
| 6 | **服务端权威校验** | 业务规则校验必须在服务端（前端只管展示/交互），前端校验只是 UX 辅助 | §6.7 |
| 7 | **LLM 成本原则** | system 必须逐字固定（动态内容进 user）、能会话化就会话化、日期只注入一次——前缀缓存命中省 ~50 倍 | §6.1 |
| 8 | **分支+验收纪律** | 每改动都走分支；**用户确认前不自动提交**；验收通过才合并 main | §4.1 |
| 9 | **Windows 命令陷阱（其他）** | `2>$null` 会生成 `$null` 文件、node -e 中文/引号被 shell 剥、curl 双引号被剥——大段替换写 .mjs 落盘、api 验证用 toolbox api | §3 |
| 10 | **测试分级** | L0 typecheck 必跑；小改动定向验证（smoke --page），**别无脑全量冒烟**；全量仅在用户要求/大需求/合并前 | §5.1 |
| 11 | **用户反馈≠实测时先自查** | 实测"正常"但用户说有问题：先查验证脚本选择器/等待、用户浏览器 HMR 未生效（硬刷新）、窗口/DPI 差异——别急着否定用户 | §5.4 弹层 |
| 12 | **改要改彻底（不留妥协）** | 改就要改彻底、先进就全部先进，**不要兼容层/双轨/过渡态**——历史遗留兼容代码在迭代中主动清除（core/tasks 退役 + AsyncTaskResult 全清除是范例）；**妥协反而混乱** | §5.0 |
| 13 | **🈲 中文提交消息必须走 `--file`** | cmd/PowerShell 以 GBK 传参给 node → `process.argv` 拿到「UTF-8 被按 GBK 解释」的乱码（"首屏"→"棣栧睆"），**写进 commit message 永久损坏历史且 Node 侧无法还原**。中文消息一律 `toolbox commit --file <UTF-8 消息文件>`；脚本已加 U+FFFD 哨兵拦截 | §4.2 |
| 14 | **批量网络取数用有界并发** | 无界 `Promise.all`（打爆对端限流）与串行 `await`（N×RTT 串联）都是反模式——统一用 `core/concurrency.ts` 的 `mapLimit(items, NET_CONCURRENCY, fn)`（**保序**）。批量接口优先（1 次请求拉 N 只），逐条仅作降级；**批量结果也要落缓存** | §6.9 |
| 15 | **性能问题先量化再动手** | 「页面加载慢」先测接口耗时 + 数取数次数定位真瓶颈，别凭直觉改渲染。冷/热缓存对比能立刻区分「网络取数」与「计算/渲染」——本次 15.8s(冷) vs 152ms(热) 直接锁定 N+1 行情取数 | §6.9 |
| 16 | **🈲 部署相关参数禁止硬编码** | 端口 / 库文件路径 / 数据目录 / 进程管理参数一律进 `toolbox.config.json`，代码里不写数字常量（硬编码的端口会让 dev 环境健康检查全程失效——本次 `dev.mjs` 写死 8787/5173 就是实例）。服务端取配置走 `core/config.ts`，脚本走 `scripts/dev-utils/config.mjs`，**禁止直接 `process.env.*`** | §6.10 |
...
3. **命令卫生（⛔ 第 1 条，最高频错误）**：bash 工具是 cmd——**写任何命令默念「不带 `;`、不 node -e 长 JS」**；能走 `toolbox` 一律走 toolbox（已自动剥尾分号），裸 git/curl 逐条写、绝不带 `;`。
4. **读历史**：先看 `docs/for_agent/history/INDEX.md`（主题索引）或最近一份归档，了解已完成/遗留（§8）。
5. **读备忘录（每会话必做）**：`toolbox memo stats` + `list`——**有 open 必须先处理**（完整流程见 §8.0；收尾前必须 open=0，否则不许提交）。
6. **建分支**：需要改动一律 `git switch -c <type>/<简述>`（§4.1，禁止 main 直改）。
7. **验证→报告→等确认后提交**：L0 typecheck → 按 §5.1 分级验证 → **报告给用户（不自动提交）** → 用户确认分支处理正确后 → `toolbox commit "msg"`（自动 add+commit+push）→ 等用户验收再合并 main（§4）。
...
**改造原则（2026-08-26 用户确立，强制——"改就要改彻底，先进就全部先进，不要妥协"）**：
- 改就要改彻底：不留兼容层、不留双轨、不留过渡态——**历史遗留的兼容代码在迭代中主动清除**，而不是"先留着以后再说"（范例：core/tasks 退役 + AsyncTaskResult/taskStatus 全清除，data-infra 成为唯一任务链路）
- 要先进就全部先进：新范式一旦选定，**所有相关代码一次到位**（同模式全量迁移，不部分迁移部分保留）
- **妥协反而混乱**：兼容层 = 两套契约 = 每处调用都要决策走哪条路——这是比"多改几行"更大的长期成本
...
### 6.9 性能：批量取数与并发（2026-09-03，仓位页首屏 15.8s→0.8s 沉淀）

**先量化再动手**：页面慢先测**接口耗时 + 取数次数**，别凭直觉改渲染。
判别法：**冷/热缓存对比**——冷 15.8s / 热 152ms ⇒ 瓶颈在网络取数，与计算/渲染无关。

**批量网络取数三原则**：

1. **批量接口优先**：一次请求拉 N 只（腾讯 `qt.gtimg.cn/q=` 批量），逐条仅作失败降级。
   逐条请求 = N 次往返，是 N+1 的首因。
2. **有界并发**：统一用 `core/concurrency.ts` 的 `mapLimit(items, NET_CONCURRENCY, fn)`（**保序返回**）。
   - 无界 `Promise.all`：上百并发打爆对端限流，失败率与长尾齐升
   - 串行 `await`：N 次 RTT 串联（本例降级路径 15s+ 主因）
3. **批量结果也要落缓存**：只缓存单源降级结果 → 批量命中的每次冷启都要重跑（本例已修复）。

**批量接口契约**：`getQuoteSnapshots(codes)` **保序**（返回数组与入参下标一一对应），
调用方按下标配对回输入 code。⚠️ 原实现先推缓存命中、后推未命中，顺序与入参不一致，无法配对——**批量接口必须保序**。

**派生复用**：批量场景（如总览算 N 个分组）用 `xxxFrom(allEntries, groupId)` 复用已加载数据，
避免每个分组各重读一次全量存储（组数 × 全量 IO）。

**前端加载**：
- 请求去重：`useRef` 记录已加载 id，消除多个 effect 各自触发的重复请求
- 竞态保护：请求序号比对，丢弃过期响应（快速切换时旧响应后到会覆盖新数据）
- 渐进加载：主数据就绪即可交互，次级数据独立加载态 + 提示，不整体白屏

### 6.10 配置化（2026-09-04，服务配置集中治理沉淀）

**原则**：**部署与服务管理相关的一切进配置文件**，代码里不留数字常量。
判断标准——「换台机器 / 换个端口 / 换个库文件部署，需要改代码吗？」需要就是没配置化。

**配置文件与优先级**（后者覆盖前者）：

```
内置默认值 → toolbox.config.json（提交，部署基线）→ toolbox.config.local.json（本地私有，gitignore）
           → TOOLBOX_CONFIG_FILE 指向的文件 → 环境变量（CI/临时覆盖）
```

**落地要点**：

1. **唯一实现，两端共用**：配置内核放 `packages/shared/config.mjs`（纯 ESM 零依赖），
   服务端（TS）与 `scripts/*.mjs`（纯 node）都引它——否则「两份默认值各改一半」必然漂移。
   类型在 `config.d.mts`（`.mjs` 与 `.d.mts` 同名即被 TS 识别）。
2. **配置写错必须启动即失败**：字段拼错 / 类型错 / 端口越界 → 抛错并指出**来源文件**。
   静默忽略拼错字段是最难排查的故障（改了配置"没生效"）。
3. **只放真正被消费的开关**：没人读的配置项等于谎言。
4. **路径语义要写清**：相对路径相对仓库根，**绝对路径直接胜出**（dev 环境注入绝对路径靠这条）；
   `server.dbFile` 相对 `server.dataDir`，写绝对路径则整体指向别处。
5. **惰性加载配置**：脚本里**不能在模块顶层求值**——调用方可能在 import 之后才回填 `process.env`
   （`dev.mjs` 命令行 `KEY=VALUE` 回填就是这种时序）。顶层求值会让覆盖静默失效。
6. **启动自报**：日志打印生效配置来源 + 库文件绝对路径；排障不用猜"到底读的哪个库"。
   配套 `toolbox config show|paths|check`（见 commands.md）。

**红线**：服务端业务代码禁止直接读 `process.env.*`（散落的 env 读 = 隐性配置，不可见也不可治理），
一律走 `core/config.ts`。
...
- **行为/语义类 memo 强制澄清（2026-08-17 mswvp08f 教训）**：拿到 memo 先辨类型——明确 UI/功能类直接实现；**行为/语义类**（"均价改变"/"卡死"/"数据不对"）**先确认用户预期的期望行为**（必要时 ask 澄清），**绝不能自证"标准行为/逻辑正确"后直接 done**——mswvp08f 用户要的是"区分买入均价与成本均价"（后补 costAvg），不是"卖出不改均价"的说明
