...
| **规范层（必读）** | `dev.md` | 开发规范总纲：分层架构 / 契约驱动 / 验证分级 / git 分支 / 本地数据 / LLM 成本原则 | 每个新会话（§0-§5 必读，§6-§8 按需） |
...
| `domains/features.md` | 浏览器自动化 / 策略仓位管理 / 数据可信度（cbRate） | 改 browserChat / trade-plan / LLM 结构化输出 |
