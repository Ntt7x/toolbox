...
- **日 K 线（core/kline.ts，2026-08-16 起）**：`fetchKlinesForCodes` 批量拉历史日 K
  （腾讯 `web.ifzq.gtimg.cn/appstock/app/fqkline/get` day 周期，qfq 前复权；行格式
  `[日期, 开, 收, 高, 低, 量]`）；KV 缓存 `kline:d:` 6 小时（历史数据稳定 + 增量合并）；
  trade-v2 收益曲线用它做真实市值口径（无行情回退成本口径）；拉取失败静默不阻塞
